...
namespace NetCarto.SQL.Functions
{
    public class Isoline
    {
        ...
        /// <returns></returns>
        public static string IsoDistance(Point point, Mode mode, int[] range)
        {
            return String.Format("cdb_isodistance('{0}'::geometry, '{1}', ARRAY[{2}]::integer[])", point, mode, String.Join(",", range));
        }
        ...
        /// <returns></returns>
        public static string IsoDistance(Point point, Mode mode, int[] range, Options options)
        {
            return String.Format("cdb_isodistance('{0}'::geometry, '{1}', ARRAY[{2}]::integer[], ARRAY[{3}]::text[])", point, mode, String.Join(",", range), options.ToString());
        }
        ...
        /// <param name="range">Range of the isoline, in meters</param>
        /// <returns></returns>
        public static string IsoChrone(Point point, Mode mode, int[] range)
        {
            return String.Format("cdb_isochrone('{0}'::geometry, '{1}', ARRAY[{2}]::integer[])", point, mode, String.Join(",", range));
        }
        ...
        /// <param name="options">Multiple options to add more capabilities to the analysis.</param>
        /// <returns></returns>
        public static string IsoChrone(Point point, Mode mode, int[] range, Options options)
        {
            return String.Format("cdb_isochrone('{0}'::geometry, '{1}', ARRAY[{2}]::integer[], ARRAY[{3}]::text[])", point, mode, String.Join(",", range), options.ToString());
        }
